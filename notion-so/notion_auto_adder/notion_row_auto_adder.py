...
def auto_add_table_row():
    ...
    time_now = f"{now.year}_{now.month}_{now.day}_{now.hour}_{now.minute}_{now.second}"
    PATH = f"C:/notion_auto_adder/screenshots/{time_now}.png"
    ...
    screenshot.upload_file(PATH)  # 로컬에 저장된 스크린샷 업로드
